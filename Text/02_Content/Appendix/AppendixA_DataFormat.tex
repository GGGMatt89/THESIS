...
\caption[Signal detection on one ASM card and logical signal sent to the THOR card.]{An ASM card detects an interaction and sends a logical signal to the THOR card.}
...
\caption[Pre-trigger generation in the THOR card.]{The THOR card generates the pre-trigger signal.}
...
\caption[Pre-trigger sent to the 10 silicon FE cards.]{The pre-trigger is sent to the 10 silicon FE cards.}
...
\caption[A single silicon FE card generates the trigger.]{A single silicon FE card generates the trigger.}
...
\caption[The detectors send the collected data to the \charmu-TCA.]{The detectors send the collected data to the \charmu-TCA.}\label{chapappA::subfig::triggerLogic_6}
\end{subfigure}%
...
\subsubsection{Monitoring process (Front End cards $\rightarrow$ \charmu-TCA)}\label{chapappA::subsubsec::monitorProcess}
...
\subsubsection{Special command process (\charmu-TCA $\rightarrow$ Front End cards)}\label{chapappA::subsec::specCmdProcess}
...
\subsection{Data packets (Front End card $\rightarrow$ \charmu-TCA)}\label{chapappA::subsec::dataPackets}
...
\subsection{Acquisition data flow (\charmu-TCA $\rightarrow$ Acquisition PC)}\label{chapappA::subsec::daqDataFlow}
