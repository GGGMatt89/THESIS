...
\newline
In \tablename~\ref{tab::FE_numbers} the Front End number IDs are listed with the corresponding cards.
...
\begin{minipage}{0.47\textwidth}
 \centering
\includegraphics[width=0.7\textwidth]{03_GraphicFiles/appendixA_dataFormat/triggerLogic_1.pdf}
\subcaption[Signal detection on one ASM card and logical signal sent to the THOR card.]{An ASM card detects an interaction \\ and sends a logical signal to the THOR card.}\label{triggerLogic_1}
\end{minipage}%
\begin{minipage}{0.47\textwidth}
 \centering
\includegraphics[width=0.7\textwidth]{03_GraphicFiles/appendixA_dataFormat/triggerLogic_2.pdf}
\subcaption[Pre-trigger generation in the THOR card.]{The THOR card generates \\ the pre-trigger signal.}\label{triggerLogic_2}
\end{minipage}%
\newline
\begin{minipage}{0.47\textwidth}
 \centering
\includegraphics[width=0.7\textwidth]{03_GraphicFiles/appendixA_dataFormat/triggerLogic_3.pdf}
\subcaption[Pre-trigger sent to the 10 silicon FE cards.]{The pre-trigger is sent \\ to the 10 silicon FE cards.}\label{triggerLogic_3}
\end{minipage}%
\begin{minipage}{0.47\textwidth}
 \centering
\includegraphics[width=0.7\textwidth]{03_GraphicFiles/appendixA_dataFormat/triggerLogic_4.pdf}
\subcaption[A single silicon FE card generates the trigger.]{A single silicon FE card \\ generates the trigger.}\label{triggerLogic_4}
\end{minipage}%
\newline
\begin{minipage}{0.47\textwidth}
 \centering
\includegraphics[width=0.7\textwidth]{03_GraphicFiles/appendixA_dataFormat/triggerLogic_5.pdf}
\subcaption[Trigger signal is distributed to all the detectors.]{The trigger signal is distributed \\ to all the detectors.}\label{triggerLogic_5}
\end{minipage}%
\begin{minipage}{0.47\textwidth}
 \centering
\includegraphics[width=0.7\textwidth]{03_GraphicFiles/appendixA_dataFormat/triggerLogic_6.pdf}
\subcaption[The detectors send the collected data to the \charmu-TCA.]{The detectors send \\ the collected data to the \charmu-TCA.}\label{triggerLogic_6}
\end{minipage}%
...
\subsubsection{Possible control symbols}

In the following table, all the possible control symbols are listed (defined by Carlos Abellan).

\begin{table} [!htbp]
\centering
\caption{Control symbol definition.}
\label{tab::ControlSymbol}
\begin{tabular}{m{1.5cm} M{2.5cm} M{2.5cm} M{4cm}}
\toprule
\rowcolor{myColorMainA!20}
...
\item For the control/command operations, here are some examples: register configuration,  FPGA dynamical programming, monitoring, etc.\newline
\end{itemize}
...
\underline{Acknowledgement packet (Front End cards $\rightarrow$ \charmu-TCA)}\newline
...
\underline{Writing register process (\charmu-TCA $\rightarrow$ Front End cards)}\newline
...
\underline{Reading register process (\charmu-TCA $\rightarrow$ Front End cards)}\newline
...
\begin{table} [!htbp]
  \centering
  \caption{Two special registers(\charmu-TCA $\rightarrow$  Front End cards)}
...
\begin{table} [!htbp]
  \centering
  \caption{Measurement packet (Front End cards $\rightarrow$ \charmu-TCA)}
\begin{tabular}{m{1.5cm} M{2cm} M{3cm} M{5.5cm}}
\toprule
\rowcolor{myColorMainA!20}
...
\subsubsection{Monitoring process (Front End cards $\rightarrow$ \charmu-TCA)}
...
\textbf{Word}  			& 	\textbf{1$\mathrm{^{st}}$ byte}	& \textbf{2$\mathrm{^{nd}}$ byte} & \textbf{Comment} \\
\midrule
	1				&	K.28.4.		& Front End number + 1 parity bit & Message in \tablename~\ref{tab::monitoringMess}.\\
\cmidrule{2-4}
	2				&	 \multicolumn{2}{M{5cm}}{15 bits for the message + 1 parity bit} 	& Message in \tablename~\ref{tab::monitoringMess}.\\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Special command process (\charmu-TCA $\rightarrow$ Front End cards)}
...
\midrule
	...
	6				&	Veto						& "0000 0000 0000  011" 	& Example: \charmu-TCA cannot receive the data. Acknowledgement packet needed\\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Data packets (Front End card $\rightarrow$ \charmu-TCA)}
...
\textbf{Single rate BGO (Hz) - 96 blocks}		& 	$7,8\times10^{7}$& $1,4\times10^{6}$ & $3,9\times10^{5}$&$1,4\times10^{5}$ &/&/\\
\textbf{Single rate BGO (Hz) - 1 block}		& 	$8,1\times10^{5}$& $1,5\times10^{4}$ & $4\times10^{3}$&$1,5\times10^{3}$ &/&/\\
\textbf{Single rate BGO (Hz) - 1 ASM card (6 blocks)}		& 	$6,5\times10^{6}$& $1,2\times10^{5}$ & $3,2\times10^{4}$&$1,2\times10^{4}$ &/&/\\
\bottomrule
\end{tabular}
...
\subsection{Data flow (Front End cards  $\rightarrow$ \charmu-TCA)\newline}
...
\textbf{BGO data flux (Mbits/s) - 96 blocks}		& 	$1,7\times10^{5}$& 373 & 873&37,3\\
\textbf{BGO data flux (Mbits/s)- 1 block}		& 	$1,7\times10^{3}$& 3,88 & 8,73&0,3 \\
\textbf{BGO data flux (Mbits/s) - 1 carte ASM}		& 	$1,4\times10^{4}$& 31,1 & 69,9&3,1\\
...
\subsection{Acquisition data flow (\charmu-TCA $\rightarrow$ Acquisition PC)}
